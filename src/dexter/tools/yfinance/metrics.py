...
def yf_get_financial_metrics_snapshot(ticker: str) -> dict:
    ...
    snapshot = {
        "ticker": ticker.upper(),
        "currency": (
            getattr(fast_info, "currency", None) if fast_info else info.get("currency")
        ),
        "market_cap": to_python(
        ...
        "fifty_day_average": to_python(
            getattr(fast_info, "fifty_day_average", None)
            if fast_info
            else info.get("fiftyDayAverage")
        ),
        "two_hundred_day_average": to_python(
            getattr(fast_info, "two_hundred_day_average", None)
            if fast_info
            else info.get("twoHundredDayAverage")
        ),
        "year_high": to_python(
            getattr(fast_info, "year_high", None)
            if fast_info
            else info.get("fiftyTwoWeekHigh")
        ),
        "year_low": to_python(
            getattr(fast_info, "year_low", None)
            if fast_info
            else info.get("fiftyTwoWeekLow")
        ),
...
def _compute_period_metrics(
    ...
    operating_income = safe_get(
        income_frame, ["OperatingIncome", "operatingIncome"], column
    )
    ...
    operating_cash_flow = safe_get(
        cashflow_frame, ["OperatingCashFlow", "operatingCashflow"], column
    )
...
def yf_get_financial_metrics(
    ...
    for column in columns:
        metrics = _compute_period_metrics(
            income_frame, balance_frame, cashflow_frame, column, shares_outstanding
        )
        record = {
    ...
    if report_period:
        records = [
            record for record in records if record.get("period") == report_period
        ]
