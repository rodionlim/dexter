...
def _select_filing(
    filings: Iterable[dict], filing_type: str, predicate
) -> Optional[dict]:
    matches = [
        filing
        for filing in filings
        if filing.get("type") == filing_type and predicate(filing)
    ]
    if not matches:
        return None
...
def _extract_items(
    text: str, items_map: dict[str, str], request_filter: Optional[list[str]] = None
) -> list[dict]:
    if not text:
        return []
    ...
    if request_filter:
        ...
        results = [
            item
            for item in results
            if item["number"].upper().replace(" ", "") in normalized
        ]
...
def yf_get_10K_filing_items(
    ...
    filing = _select_filing(
        filings,
        "10-K",
        lambda f: isinstance(f.get("date"), date) and f.get("date").year == year,
    )
    if not filing:
        return {
