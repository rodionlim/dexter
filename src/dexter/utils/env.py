...
def ensure_api_key_for_model_provider(model_provider: MODEL_PROVIDER) -> bool:
    """
    Ensure that the required API key exists for a given model provider.
    If it doesn't exist, prompt the user to enter it and save it to .env.

    Args:
        model_provider: The model provider to check

    Returns:
        True if the API key exists or was successfully saved, False otherwise
    """
    api_key_name = "LLM_API_" + model_provider.upper() + "_KEY"
